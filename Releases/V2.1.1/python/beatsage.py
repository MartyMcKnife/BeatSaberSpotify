...
class BeatSage:
    ...
    def request_song(self,songCover, songTitle, songArtist, path, unzip):
        if not os.path.isdir(self.folder_path) and not os.path.isfile(self.folder_path + ".zip"):
            ...
            if youtubeUrl != None:
                ...
                if download.status_code == 200:
                    ...
                    self.logger.warning(f"BeatSage had bad response code with {songTitle} and URL: {youtubeUrl}. Server Response Code: {download.status_code}")
                    print(f"BeatSage had bad response code with {songTitle} and URL: {youtubeUrl}. Server Response Code: {download.status_code}. Skipping for now")
                    return None
            else:
                return None
        else:
