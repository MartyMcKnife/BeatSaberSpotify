...
class BeatSage:
    def __init__(self, songTitle, songArtist, path):
        ...
        self.folder_path = os.path.join(
            path, "BeatSage ({0} - {1})".format(songTitle, songArtist)
        )
        self.zipPath = self.folder_path + ".zip"
        self.logger = l.create_logger()
        self.logger.info(f"Folder Path: {self.folder_path}".encode("utf-8"))
    ...
    def request_song(self, songCover, songTitle, songArtist, path, unzip):
        if not os.path.isdir(self.folder_path) and not os.path.isfile(
            self.folder_path + ".zip"
        ):
            youtubeUrl = yt.scrape_songs_from_youtube(songTitle, songArtist)
            self.logger.info(f"YoutubeUrl: {youtubeUrl}")
            ...
            if youtubeUrl != None:
                ...
                idResponse = requests.post(
                    self.createUrl, data=obj, headers=self.headers
                )
                ...
                while (
                    json.loads(
                        (requests.get(self.heartBeatUrl + id, self.headers)).text
                    )["status"]
                    == "PENDING"
                ):
                    print(
                        "Waiting for BeatSage to create the level, job order is {0}".format(
                            id
                        ),
                        flush=True,
                    )
                ...
                if download.status_code == 200:
                    with open(self.zipPath, "wb+") as f:
                        self.logger.info(
                            f"Downloading song, status code is {download.status_code}"
                        )
                    ...
                    if unzip == True:
                        with ZipFile(self.zipPath, "r") as z:
                            ...
                            shutil.copy(
                                songCover, os.path.join(self.folder_path, "cover.jpg")
                            )
                    ...
                    print(
                        "Downloaded {0}!".format(songTitle).encode("utf-8"), flush=True
                    )
                    return "done"
                else:
                    self.logger.warning(
                        f"BeatSage had bad response code with {songTitle} and URL: {youtubeUrl}. Server Response Code: {download.status_code}"
                    )
                    print(
                        f"BeatSage had bad response code with {songTitle} and URL: {youtubeUrl}. Server Response Code: {download.status_code}. Skipping for now"
                    )
                    return None
            else:
                return None
            ...
            print(
                "Song {0} is already generated. Skipping".format(songTitle).encode(
                    "utf-8"
                ),
                flush=True,
            )
