...
class BeatSaberSpotify:
    def __init__(self, root_path, username, headsetType):
        ...
        self.logger.info(
            f"Download Directory: {self.download_directory}".encode("utf-8")
        )
        self.logger.info(
            f"Custom Songs Directory: {self.custom_songs_directory}".encode("utf-8")
        )
        self.logger.info(
            f"Custom Playlists Directory: {self.custom_playlists_directory}".encode(
                "utf-8"
            )
        )
        self.logger.info(f"Unzip?: {self.unzip}")

        if not os.path.isdir(self.custom_songs_directory) and self.unzip == True:
            raise OSError(
                "Could not find custom songs directory. Make sure you have the right path and try again"
            )
    ...
    def SpotifyWriter(self, username, playlist_id):
        ...
        self.artist_file = spotify.write_playlist(
            username, playlist_id, self.download_directory
        )
        self.track_file = self.artist_file.replace("- Artists.txt", "- Tracks.txt")
        self.playlist_name = self.artist_file.replace(" - Artists.txt", "")
        image = spotify.get_playlist_image(
            playlist_id, self.download_directory, self.playlist_name
        )
        self.encoded_image = spotify.base64_encode(
            os.path.join(self.download_directory, image)
        )
        self.jsonFile = os.path.join(
            self.custom_playlists_directory, "{0}.json".format(self.playlist_name)
        )
        os.remove(image)
    ...
    def youtubeWriter(self, playlist_id):
        ...
        self.playlist_name, playlistResp = y.scrape_playlist(
            playlist_id, self.download_directory
        )
        self.track_file = self.playlist_name + "- Tracks.txt"
        self.artist_file = self.playlist_name + "- Artists.txt"
        image = y.getCoverImage(
            playlistResp, self.playlist_name, self.download_directory
        )
        self.encoded_image = y.base64_encode(
            os.path.join(self.download_directory, image)
        )
        self.jsonFile = os.path.join(
            self.custom_playlists_directory, "{0}.json".format(self.playlist_name)
    ...
    def downloadSong(self, songName, artistName, got_songs, songlist, locker):
        print(
            "Grabbing {0} by {1}".format(songName, artistName).encode("utf-8"),
            flush=True,
        )
        self.logger.info(
            "Grabbing {0} by {1}".format(songName, artistName).encode("utf-8")
        ...
        self.logger.info(f"Song Cover: {songCover}".encode("utf-8"))
        bsSongId, bsSongName, bsUsername = downloader.get_song_info(
            songName, artistName
        )
        self.logger.info(
            f"BeatSaver Song Info for {songName}: {bsSongId}, {bsSongName}, {bsUsername}".encode(
                "utf-8"
            )
        )

        if bsSongId != None:
            downloader.download_song_from_id(
                bsSongId,
                bsSongName,
                bsUsername,
            ...
            downloaded = ai.BeatSage(
                songName, artistName, self.custom_songs_directory
            ).request_song(
                songCover, songName, artistName, self.custom_songs_directory, self.unzip
            )
            if downloaded == None:
                bsSongId = None
            else:
        ...
        if bsSongId != None:
            got_songs.value += 1
            print("Current" + str(got_songs.value), flush=True)
            for char in self.invalid:
                bsSongName = bsSongName.replace(char, "")
            hash = idgrabber.get_id(
                os.path.join(
                    self.custom_songs_directory,
                    "{0} ({1} - {2})".format(bsSongId, bsSongName, bsUsername),
                ),
                self.unzip,
            )
    ...
    def addToJson(
        ...
        if write == True:
            ...
            with open(self.jsonFile, "r+") as f:
                songlist["songs"].append({"hash": hash})
    ...
    def run(self, username, playlist_id):
        ...
        with open(self.artist_file, "r", encoding="utf-8") as a:
            with open(self.track_file, "r", encoding="utf-8") as t:
                ...
                print(
                    "Collecting songs from BeatSaber and BeatSage. This can take a while, depending on the size of the playlist",
                    flush=True,
                )
                items = list(zip(tracks, artists))
                self.logger.info(f"Items to download: {items}".encode("utf-8"))
                ...
                print(
                    "Got {0} of {1} songs".format(got_songs.value, self.total_songs),
                    flush=True,
                )
